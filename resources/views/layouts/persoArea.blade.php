    ...
    <div class="row">
        <div class="col-lg-4 col-md-12 min-height">
            ...
            <div class="bg-white px-5 py-5 mt-2 shadow-sm">
                @if($user->role_id == 2)
                <a href="{{ route('professionnelArea.indexAgenda', Auth::user()->id) }}"
                    class="btn btn-pr btn-block">Mon agenda</a>
                <a href="{{ route('professionnelArea.indexAppointment', Auth::user()->id) }}"
                    class="btn btn-pr btn-block">Mes rendez-vous</a>
                <a href="{{ route('professionnelArea.edit', Auth::user()->id) }}" class="btn btn-pr btn-block">Mes
                    informations personnelles</a>
                @else
                <a href="{{ route('clientArea.index', Auth::user()->id) }}" class="btn btn-pr btn-block">Mes
                    rendez-vous</a>
